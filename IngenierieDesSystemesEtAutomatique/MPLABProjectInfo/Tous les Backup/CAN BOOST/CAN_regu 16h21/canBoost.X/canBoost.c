    ...
    * to catch the next message. */
    ...
    //! CONSIGNE OU REQUETE
        else if((message->msgSID.SID) == idModule.SID)
        {
            ...
            {
                float tabValSortie[] = {0.0,18.0,24.0,30.0,36.0};
                putUartStr("Consigne : ");
                consigneBoost = message->data[0];
                if(consigneBoost==0 || consigneBoost>4){
                    consigneBoost=2;
                }
                consigneDuCan = tabValSortie[consigneBoost];
        ...
        * This is the payload. */
        ...
        switch(typeReq)
        {
            ...
            case REPONSE:   message->msgSID.SID     = filtreBoost;
                            ...
                            message->data[0]        = (char)consigneDuCan;
                            message->data[1]        = voBoostToCan;
                            message->data[2]        = viBoostToCan;
                            message->data[3]        = vilBoostToCan;
                            break;
