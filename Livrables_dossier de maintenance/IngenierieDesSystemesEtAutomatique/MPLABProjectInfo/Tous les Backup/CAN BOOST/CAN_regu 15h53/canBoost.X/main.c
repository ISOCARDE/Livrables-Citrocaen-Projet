...
int main(void)
{
    ...
    flagTraitement = 0;
    consigneDuCan = 24;
    int tempovo = 0b0011001100000000;
    tempovo=tempovo>>6;
    voBoostToCan = (BYTE)tempovo;
    viBoostToCan = 8;
    vilBoostToCan = 9;

    unsigned char buffer[10];
    float stock = (float)(voBoostToCan);
    BYTE tem = (BYTE) tempovo;
    snprintf(buffer, 10, "%f", stock);

    while(1)
    {
        putUartC(buffer); putUartC('\r'); putUartC('\n');
