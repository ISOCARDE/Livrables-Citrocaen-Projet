...
void ad7885Pic32Read(SpiChannel CH, float* data, SlaveS SS){
    ...
    //for(i=0;i<1000;i++);
    tempo = (SpiChnGetC(CH)); //SPI3ABUF;
    Nop();
    tempo=tempo>>6;
    Nop();
    //for(i=0;i<1000;i++);
    PORTSetBits(SS.Port,SS.Bit);
    *data = (float)10*tempo*0.0195;     //si changement, prévenir M.WERCK
    SpiChnGetRov(CH, TRUE);
    /*partie a commenter pour emulateur*/
//    if(SS.Bit == BIT_3){
//        vilBoostToCan = (BYTE)(tempo);
//    }
//    else if(SS.Bit == BIT_14){
//        voBoostToCan = (BYTE)(tempo);
//    }
//    else{
//        viBoostToCan = (BYTE)(tempo);
//    }
